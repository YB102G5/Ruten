...
def freq2file(freqDict, file):
    ...
    for key in dict:
        ...
        f2.write('\n')
    f2.close()
    #呼叫一次freq2file func就要做兩次IO

def append_word_dict(text, dict): #做斷詞、統計頻率和寫入詞庫檔案
    text_clean = re.sub(cleanCom, '|', text)
    segmentList = text_clean.split('|')
    list = []
    for segment in segmentList:
        for i in segment.split():
            list.append(i)
    for segment in list:
        if bool(re.match("^[a-zA-Z'\.]+$", segment)): #只針對segment為純英文字
            dict[segment] = dict.get(segment, 0)+1

        freqDict = ngram_wo_1gram(segment, 4)
        for key in freqDict:
            dict[key] = dict.get(key, 0)+freqDict[key]
...
f.close()
rowCount = 1
wordDictFile = 'word_dict_test4.txt'
wordDict = {}
startTime = int(round(time.time() * 1000))
for row in list: #商品內容表格row
    try:
        title = row.split(',')[0]
        append_word_dict(title, wordDict) #每個商品都要把詞庫打開建立詞庫dict再把斷詞詞頻加入
    ...
    rowCount += 1
freq2file(wordDict, wordDictFile)
